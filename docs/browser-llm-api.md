...
# TechieMind Browser LLM API Documentation (Beta)

## Overview

> Please note that this API is currently in beta and is subject to change.

The `navigator.llm.responses` API is a powerful browser-native interface that brings Large Language Model capabilities directly to web applications. Built into the TechieMind extension, this API provides seamless access to local AI models without requiring external API keys or cloud dependencies.

### How It Works

The API acts as a bridge between your web application and locally running AI models managed by TechieMind. When you make a request:
...
The API extends the native `navigator` object, making it available globally in any web page where the TechieMind extension is active. No additional libraries or authentication required - just start using `navigator.llm.responses.create()` in your JavaScript code.

## Usage Examples

### Feature Detection

Before using the API, it's recommended to check if the TechieMind extension is available and the API is accessible:

```javascript
// Check if the TechieMind LLM API is available
if (navigator.llm?.responses?.create) {
  // TechieMind extension is available, proceed with API usage
} else {
...
| `model`       | `string`         | No       | Model identifier to use for generation, which should be downloaded and available in the TechieMind settings. default to value used in TechieMind settings |
...
1. The TechieMind container will be toggled to show
...
This API is available when the TechieMind extension is installed and active. It extends the native `navigator` object with LLM capabilities.

## Notes

- All requests are processed locally through the TechieMind backend
